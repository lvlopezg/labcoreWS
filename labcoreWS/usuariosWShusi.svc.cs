...
INNER JOIN hcePersonal B ON A.IdUsuario = B.IdPersonal
 WHERE A.idUsuario = " + Idusuario;  //+@" AND A.ind_esta = 'A'"  se quita la condicion, y se retorna el estado en la respuesta
        ...
        if (rdConsulta.HasRows)
        {
          ...
          rdConsulta.Read();
          if (rdConsulta.IsDBNull(0))
          { codigo = ""; }
          else
          { codigo = rdConsulta.GetString(0); }
          if (rdConsulta.IsDBNull(1))
          { usrwindows = ""; }
          else
          { usrwindows = rdConsulta.GetString(1); }
          if (rdConsulta.IsDBNull(2))
          { nombre = ""; }
          else
          { nombre = rdConsulta.GetString(2); }
          if (rdConsulta.IsDBNull(3))
          { nroRegistro = ""; }
          else
          { nroRegistro = rdConsulta.GetString(3); }
          if (rdConsulta.IsDBNull(4))
          { estado = ""; }
          else
          { estado = rdConsulta.GetString(4); }
...
INNER JOIN hcePersonal B ON A.IdUsuario = B.IdPersonal
 WHERE B.idPersonal =@idPersonal";  //+@" AND A.ind_esta = 'A'"  se quita la condicion, y se retorna el estado en la respuesta
        SqlCommand cmdConsulta = new SqlCommand(qryConsulta, DBConexion);
        cmdConsulta.Parameters.Add("@idPersonal", SqlDbType.VarChar).Value = idPersonal;
        SqlDataReader rdConsulta = cmdConsulta.ExecuteReader();
        if (rdConsulta.HasRows)
        {
          string codigo = string.Empty;
